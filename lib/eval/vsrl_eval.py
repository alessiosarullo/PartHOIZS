...
def pkl_from_predictions(dict_predictions: List[Dict], dataset: VCocoSplit, filename: str):
    ...
    pred_list = []
    for im_idx, im_dict_pred in enumerate(dict_predictions):
        im_id = dataset.full_dataset.get_fname_id(dataset.all_gt_img_data[im_idx].filename)
        pred = Prediction(im_dict_pred)
        if pred.ho_pairs is None:
            continue

        ext_act_scores_per_human = {}
        for (h_idx, o_idx), act_scores in zip(pred.ho_pairs, pred.output_scores):
            ext_act_scores_per_human.setdefault(h_idx, []).append(np.append(o_idx, act_scores))
        ext_act_scores_per_human = {k: np.stack(v, axis=0) for k, v in ext_act_scores_per_human.items()}

        obj_inds_and_act_scores_per_human = {}
        for h_idx, ext_act_scores in ext_act_scores_per_human.items():
            obj_inds = ext_act_scores[:, 0].astype(np.int)
            act_scores = ext_act_scores[:, 1:]

            most_likely_obj_per_action = np.argmax(act_scores, axis=0)
            assigned_act_scores = np.max(act_scores, axis=0)
            assigned_obj_inds = obj_inds[most_likely_obj_per_action]
            obj_inds_and_act_scores_per_human[h_idx] = (assigned_obj_inds, assigned_act_scores)

        for h_idx, (obj_inds, act_scores) in obj_inds_and_act_scores_per_human.items():
            im_pred_vcoco = {'image_id': im_id,  # the coco image id,
                             'person_box': pred.obj_boxes[h_idx, :],  # [x1, y1, x2, y2],
                             }
            agent_score = {}
            for j, (o_idx, s) in enumerate(zip(obj_inds, act_scores)):
                if j == 0:
                    continue
                a_r = actions[j]
                im_pred_vcoco[f'{a_r}'] = np.append(pred.obj_boxes[o_idx, :], s)  # [x1, y1, x2, y2, s]

                # Assign maximum score to agent action
                a = '_'.join(a_r.split('_')[:-1])
                curr_score = agent_score.setdefault(a, 0)
                if s > curr_score:
                    agent_score[a] = s

            for a, s in agent_score.items():
                im_pred_vcoco[f'{a}_agent'] = s  # note: only used for AP_act

            pred_list.append(im_pred_vcoco)
...
if __name__ == '__main__':
    VCOCOeval(vsrl_annot_file='data/V-COCO/vcoco/vcoco_test.json',
              coco_annot_file='data/V-COCO/instances_vcoco_all_2014.json',
              split_file='data/V-COCO/splits/vcoco_test.ids')
