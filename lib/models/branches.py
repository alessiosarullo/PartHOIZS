...
class AbstractBranch(nn.Module):
    def __init__(self, dataset: HicoHakeKPSplit, cache: Cache, repr_dim=cfg.repr_dim, enable_zs=True, wrapped_branch=None, **kwargs):
        super().__init__()
    ...
    def forward(self, x: Minibatch, inference=True, **kwargs):
        with torch.set_grad_enabled(self.training):
            ...
            output = self.forward_wrap(img_data, person_data, obj_data, orig_labels, orig_part_labels, other)
    ...
    def forward_wrap(self, img_data, person_data, obj_data, labels, part_labels, other):
        output = self._forward(img_data, person_data, obj_data, labels, part_labels, other)
        if self.wrapped_branch is not None:
            if not isinstance(output, tuple):
                output = (output,)
            output_lists = tuple([o] if not isinstance(o, list) else o for o in output)
            wrapped_output = self.wrapped_branch.forward_wrap(img_data, person_data, obj_data, labels, part_labels, other)
...
class UninteractivenessBranch(AbstractBranch):
    def __init__(self, sp_ch=64, pose_ch=32, *args, **kwargs):
        super().__init__(*args, **kwargs)
        F_kp, F_obj = self.dataset.kp_net_dim, self.dataset.obj_feats_dim  # feature vector dimensionality
        K_hake = self.dataset.full_dataset.num_keypoints  # #keypoints
        repr_dim = self.repr_dim
        D = cfg.ipsize
        d = D // 4

        self.no_interaction_inds = nn.Parameter(torch.from_numpy(self._get_no_interaction_inds()), requires_grad=False)
        self.predictor = nn.Linear(in_features=repr_dim, out_features=self.no_interaction_inds.shape[0])
        ...
        self.fc8_binary1 = nn.Sequential(nn.Linear(in_features=d * d * (sp_ch // 2 + pose_ch // 2) + F_kp, out_features=repr_dim),
                                         nn.Dropout(p=cfg.tin_dropout))
        self.fc8_binary2 = nn.Sequential(nn.Linear(in_features=F_obj + self.dataset.full_dataset.num_objects, out_features=repr_dim),
                                         nn.Dropout(p=cfg.tin_dropout))
        self.fc9_binary = nn.Sequential(nn.Linear(in_features=2 * repr_dim, out_features=repr_dim),
                                        nn.Dropout(p=cfg.tin_dropout))

    def _get_no_interaction_inds(self):
        raise NotImplementedError
    ...
    def _forward(self, img_data, person_data, obj_data, labels, part_labels, other):
        ppl_boxes, ppl_feats, coco_kps, kp_boxes, kp_feats, ipatterns = person_data[:6]
        obj_boxes, obj_scores, obj_feats = obj_data[:3]
        ...
        if cfg.tin_mean:
            logits = logits.mean(dim=2).mean(dim=1)  # N x C
        else:
            logits = logits.max(dim=2)[0].max(dim=1)[0]  # N x C
        return logits


class HoiUninteractivenessBranch(UninteractivenessBranch):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

    def _get_no_interaction_inds(self):
        return np.flatnonzero(self.dataset.full_dataset.interactions[:, 0] == 0)

    def _get_losses(self, x, output, **kwargs):
        img_data, person_data, obj_data, interaction_labels, part_labels, other = x
        labels = interaction_labels[:, self.no_interaction_inds]
        losses = {f'hoi_bg_loss': bce_loss(output, labels)}
        return losses


class PartUninteractivenessBranch(UninteractivenessBranch):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

    def _get_no_interaction_inds(self):
        return np.array([states[-1] for states in self.dataset.full_dataset.states_per_part])

    def _get_losses(self, x, output, **kwargs):
        img_data, person_data, obj_data, interaction_labels, part_labels, other = x
        labels = part_labels[:, self.no_interaction_inds]
...
class PartActionBranch(AbstractBranch):
    def __init__(self, enable_gcn=False, *args, **kwargs):
        ...
        for i, p_acts in enumerate(hicohake.states_per_part):
            ...
            if not cfg.no_kp:
                self._add_mlp(name=f'kp_part_{part_name}', input_dim=self.dataset.kp_net_dim, output_dim=self.repr_dim)
                logit_input_dim += self.repr_dim
...
class PartStateBranch(AbstractBranch):
    def __init__(self, *args, **kwargs):
        ...
        # Initialise part actions attributes according to GT.
        self.fg_part_states = nn.Parameter(torch.from_numpy(np.concatenate([app[:-1] for app in hicohake.states_per_part])), requires_grad=False)
        ...
        for i, p_states in enumerate(hicohake.states_per_part):
            ...
            self._add_mlp(name=f'img_part_{part_name}', input_dim=self.dataset.precomputed_visual_feat_dim, output_dim=self.repr_dim)
            self._add_mlp(name=f'kp_part_{part_name}', input_dim=self.dataset.kp_net_dim, output_dim=self.repr_dim)
            logit_input_dim = self.repr_dim + hicohake.num_objects + self.repr_dim  # direct repr + obj scores + aggr people repr
    ...
    def _get_losses(self, x, output, **kwargs):
        ...
        bg_part_states = self.uninteractiveness_branch.no_interaction_inds
        losses = {'fg_part_loss': bce_loss(part_dir_logits[:, self.fg_part_states], orig_part_labels[:, self.fg_part_states],
                                           pos_weights=cfg.cspc_p if cfg.cspc_p > 0 else None),
                  'bg_part_loss': bce_loss(part_dir_logits[:, bg_part_states], orig_part_labels[:, bg_part_states],
                                           pos_weights=cfg.cspbgc_p if cfg.cspbgc_p > 0 else None)
                  }
    ...
    def _predict(self, x, output, **kwargs):
        ...
        feats = img_data[1]
        part_dir_logits = output
        part_state_scores = np.zeros((feats.shape[0], self.dataset.full_dataset.num_part_states))
        if part_dir_logits is not None:
            part_state_scores = torch.sigmoid(part_dir_logits).cpu().numpy()
            part_state_scores[:, self.uninteractiveness_branch.no_interaction_inds] = self.uninteractiveness_branch(x, inference=True)
    ...
    def _forward(self, img_data, person_data, obj_data, labels, part_labels, other):
        ...
        im_part_logits_list = []
        for part_idx, p in enumerate(self.dataset.full_dataset.parts):
            p_logits = im_part_reprs[:, part_idx, :] @ self.linear_predictors[f'img_part_{p}']
            im_part_logits_list.append(p_logits)
        im_part_dir_logits = torch.cat(im_part_logits_list, dim=1)
...
class FrozenPartStateBranch(PartStateBranch):
    ...
    def _forward(self, img_data, person_data, obj_data, labels, part_labels, other):
        ...
        if epoch_idx > self.starting_epoch and all([imid in self.part_cache for imid in im_ids]):
            # The second condition might not be true in epochs after the first because of the drop_last option of the data loader.
            dir_logits, part_reprs = zip(*[self.part_cache[imid] for imid in im_ids])
            im_part_dir_logits = torch.stack(dir_logits, dim=0)
            im_part_reprs = torch.stack(part_reprs, dim=0)
            self.cache['part_dir_logits'] = im_part_dir_logits
            self.cache['im_part_reprs'] = im_part_reprs
        else:
            im_part_dir_logits = super()._forward(img_data, person_data, obj_data, labels, part_labels, other)
            im_part_reprs = self.cache['im_part_reprs']
            ...
            for i, imid in enumerate(im_ids):
                ...
                dir_logits = im_part_dir_logits[i, :]
                part_reprs = im_part_reprs[i, :, :]
                self.part_cache[imid] = [dir_logits, part_reprs]
        return im_part_dir_logits


class ZSBranch(AbstractBranch):
    def __init__(self, label_type=None, *args, **kwargs):
        super().__init__(*args, **kwargs)
    ...
    def _get_label_data(self, label_type=None) -> LabelData:
        if label_type == 'obj':
            return LabelData(tag=label_type,
                             ...
                             unseen_classes=np.setdiff1d(np.arange(self.dataset.full_dataset.num_objects), self.dataset.seen_objects),
            ...
            return LabelData(tag='act',
                             ...
                             unseen_classes=np.setdiff1d(np.arange(self.dataset.full_dataset.num_actions), self.dataset.seen_actions),
            ...
            return LabelData(tag='hoi',
                             ...
                             unseen_classes=np.setdiff1d(np.arange(self.dataset.full_dataset.num_interactions), self.dataset.seen_interactions),
    ...
    def _forward(self, img_data, person_data, obj_data, labels, part_labels, other):
        repr = self._get_repr(img_data, person_data, obj_data, other)
    ...
    def _get_repr(self, img_data, person_data, obj_data, other):
        img_feats = img_data[1]
...
class GcnObjectBranch(ZSGCBranch):
    def __init__(self, *args, **kwargs):
        super().__init__(label_type='obj', *args, **kwargs)
...
class GcnHoiBranch(ZSGCBranch):
    def __init__(self, *args, **kwargs):
        super().__init__(label_type='hoi', *args, **kwargs)
        ...
        self.seen_fg_inds = nn.Parameter(torch.from_numpy(np.intersect1d(self.ld.seen_classes,
...
class GcnActionBranch(ZSGCBranch):
    def __init__(self, *args, **kwargs):
        super().__init__(label_type='act', *args, **kwargs)
        if cfg.awsu > 0:  # if using weak supervision for unseen actions
...
class ActionFromPartBranch(GcnActionBranch):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
    ...
    def _get_repr(self, img_data, person_data, obj_data, other):
        return self.repr_mlps['part_logits_to_act'](self.cache['part_dir_logits'])


class HoiFromObjBranch(GcnHoiBranch):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
    ...
    def _forward(self, img_data, person_data, obj_data, labels, part_labels, other):
        obj_boxes, obj_scores, obj_feats = obj_data[:3]
        hoi_logits = (self.repr_mlps['obj_to_hoi'](obj_scores) @ self.linear_predictors['obj_to_hoi'])
        hoi_logits = hoi_logits.max(dim=1)[0]
        return hoi_logits, hoi_logits


class HoiFromPoseBranch(GcnHoiBranch):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        assert cfg.tin
        D = cfg.ipsize
        ...
        self._add_mlp(name='pose_to_hoi', input_dim=d * d * pose_ch // 2, output_dim=self.repr_dim,
                      num_classes=self.ld.num_classes)
    ...
    def _forward(self, img_data, person_data, obj_data, labels, part_labels, other):
        ...
        ipatterns = ipatterns.view(N * P * M, D, D, -1).permute(0, 3, 1, 2)
        pose_feats = self.pose_module(ipatterns[:, -1:, ...]).view(N * P * M, -1).view(N, P, M, -1)
        hoi_logits = (self.repr_mlps['pose_to_hoi'](pose_feats) @ self.linear_predictors['pose_to_hoi'])
        hoi_logits = hoi_logits.max(dim=2)[0].max(dim=1)[0]
        return hoi_logits, hoi_logits


class HoiFromPartBranch(GcnHoiBranch):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self._add_mlp(name='part_logits_to_hoi', input_dim=self.dataset.num_part_actions, output_dim=self.repr_dim)

    def _get_repr(self, img_data, person_data, obj_data, other):
        return self.repr_mlps['part_logits_to_hoi'](self.cache['part_dir_logits'])
