...
def bce_loss(logits, labels, pos_weights=None, reduce=True):
    if cfg.fl_gamma != 0:  # Focal loss
        ...
        if pos_weights is not None and not isinstance(pos_weights, torch.Tensor):
            assert isinstance(pos_weights, (int, float)), (type(pos_weights), pos_weights)
            pos_weights = pos_weights * torch.ones(logits.shape[1]).to(logits)
        loss = functional.binary_cross_entropy_with_logits(logits, labels, pos_weight=pos_weights, reduction='mean' if reduce else 'none')
