...
class PartActionBranch(AbstractBranch):
    ...
    def _get_repr(self, img_data, person_data, obj_data):
        # N = #images, B = #bodyparts, F = image features dim, D = part representation dim
        im_ids, feats, orig_img_wh = img_data
        ...
        if not cfg.no_kp:
            if im_inds:  # All of these are computed for each person and then aggregated to obtain one per image.
                ...
                part_kp_reprs_per_person = torch.stack(part_kp_reprs_per_person, dim=1)  # N x B x F
                ...
                if cfg.spcfmdim > 0:
                    ...
                    kp_weights_per_person = torch.stack(kp_weights_per_person, dim=1)  # N x B x 1
        ...
        assert im_part_reprs.shape[0] == feats.shape[0] and im_part_reprs.shape[1] == hh.num_parts and im_part_reprs.shape[2] == dim  # N x B x D
