...
class HicoHakeKPSplit(HicoHakeSplit):
    def __init__(self, no_feats=False, *args, **kwargs):
        ...
        try:
            ...
            # Cache
            self.pc_img_inds, imgs_with_kps = self._compute_img_data()  # type: List[Dict]
            assert len(self.pc_img_inds) == len(self.full_dataset.split_filenames[self._data_split])
            ...
            if cfg.spcfmdim > 0:
                ...
                try:
                    ...
                    for i, im_data in enumerate(self.pc_img_inds):
                        try:
