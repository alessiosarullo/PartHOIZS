...
class AbstractBranch(nn.Module):
    ...
    def _get_dims(self, img_data, person_data, obj_data):
        ...
        ppl_im_inds, ppl_boxes, coco_kps, kp_boxes, kp_feats = person_data
        P, K_hake, F_kp = kp_feats.shape
        K_coco = coco_kps.shape[1]
        assert len(ppl_im_inds) == N
        assert ppl_boxes.shape == (P, 4)
        assert coco_kps.shape == (P, K_coco, 3)
        assert kp_boxes.shape == (P, K_hake, 5)

        obj_im_inds, obj_boxes, obj_scores, obj_feats = obj_data
        M, F_obj = obj_feats.shape
        assert len(obj_im_inds) == N
        assert obj_boxes.shape == (M, 4)
        assert obj_scores.shape == (M, O)

        return Dims(N=N, P=P, M=M, K_coco=K_coco, K_hake=K_hake, B=B, O=O, F_kp=F_kp, F_obj=F_obj)

    def _compute_per_img(self, x, func, im_inds):
        return torch.stack([func(x[iids], dim=0) if iids.size > 0 else x.new_zeros(x.shape[1:])
                            for iids in im_inds], dim=0)
    ...
    def _get_repr(self, img_data, person_data, obj_data):
        ...
        if not cfg.no_kp:
            ...
            for p, kps in enumerate(hh.part_to_kp):
                ...
                part_repr = (kp_repr * kp_weights).mean(dim=1)
                ppl_part_reprs.append(part_repr)
            ppl_part_reprs = torch.stack(ppl_part_reprs, dim=1)
            assert ppl_part_reprs.shape[:-1] == (P, B)
            # im_part_reprs_from_kps = torch.stack([ppl_part_reprs[iids].mean(dim=0) if iids.size > 0 else ppl_part_reprs.new_zeros(B)
            #                                       for iids in ppl_im_inds], dim=0)
            im_part_reprs_from_kps = self._compute_per_img(x=ppl_part_reprs, func=torch.mean, im_inds=ppl_im_inds)
            im_part_reprs.append(im_part_reprs_from_kps)

            # Object detection's scores
            # im_obj_scores = torch.stack([torch.max(obj_scores[iids], dim=0)[0] if iids.size > 0 else obj_scores.new_zeros(O)
            #                              for iids in obj_im_inds], dim=0)
            im_obj_scores = self._compute_per_img(x=obj_scores, func=lambda x, **kwargs: torch.max(x, **kwargs)[0], im_inds=obj_im_inds)
            ...
            if cfg.use_sk:
                ...
                ppl_boxes_ext = ppl_boxes.unsqueeze(dim=1)  # N x 1 (bodyparts) x 4
                ...
                skeletons = norm_coco_kps.view(norm_coco_kps.shape[0], -1)
                part_specific_skeleton_reprs = torch.stack([self.repr_mlps[f'ppl_skeleton_for_{p}'](skeletons) for p in hh.parts], dim=1)
                assert part_specific_skeleton_reprs.shape[1] == B
                # im_part_specific_skeleton_reprs = torch.stack([part_specific_skeleton_reprs[iids].mean(dim=0) for iids in ppl_im_inds], dim=0)
                im_part_specific_skeleton_reprs = self._compute_per_img(x=part_specific_skeleton_reprs, func=torch.mean, im_inds=ppl_im_inds)
