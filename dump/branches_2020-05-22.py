...
class ZSBranch(AbstractBranch):
    ...
    def _init_repr(self):
        input_dim = self.dims.F_img + self.dims.F_ex + 2 * self.dims.O
...
class PartStateInReprBranch(ZSGCBranch):
    ...
    def _init_repr(self):
        input_dim, _input_repr_f = super()._init_repr()
        input_dim = input_dim + self.dims.S_sym
        self._add_mlp(name='repr', input_dim=input_dim, output_dim=self.repr_dims[0], override=True)

        def input_repr_f(x: Minibatch):
            base_repr = _input_repr_f(x)
            symstate_logits = self._get_symstate_logits()  # N x S_sym
            return torch.cat([base_repr, symstate_logits], dim=1)
...
class AttBranch(PartStateInReprBranch):
    ...
    def _init_repr(self):
        ...
        self.mha = nn.MultiheadAttention(embed_dim=self.att_emb_dim, num_heads=self.num_att_head)
        ...
        def input_repr_f(x: Minibatch):
            ...
            attn_output, attn_output_weights = self.mha(query=att_input, key=att_input, value=att_input)  # 4 x N x f, N x 4 x 4
