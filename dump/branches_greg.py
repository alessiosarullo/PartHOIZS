...
class ZSGCBranch(ZSBranch):
    def __init__(self, enable_gcn=True, gcn_tag='oa', enable_ws=None, *args, **kwargs):
        ...
        self.aa_adj = nn.Parameter((self.cache.oa_adj.t() @ self.cache.oa_adj).clamp(max=1).bool(), requires_grad=False)
    ...
    def _get_losses(self, x: Minibatch, output, **kwargs):
        losses = super()._get_losses(x, output)
        # reg loss
        if self.ld.tag == 'act' and cfg.agreg > 0:
            reg_loss = self.get_reg_loss()
            losses[f'act_reg_loss'] = cfg.agreg * reg_loss
        return losses

    def get_reg_loss(self):
        seen = self.seen_inds
        unseen = self.unseen_inds
        predictors = self._get_zs_predictor().t()
        assert predictors.shape[0] == self.dims.A
        ...
        reg_loss_mat = (0.3 - predictors_sim_diff).clamp(min=0)  # 0.3 = margin
        ...
        # null = ~adj.any(dim=1)
        #
        # predictors_sim = predictors_sim[unseen, :]
        # adj = adj[unseen, :]
        # null_u = ~adj.any(dim=1)
        # arange_u = torch.arange(predictors_sim.shape[0])
        #
        # predictors_sim_diff = predictors_sim.unsqueeze(dim=2) - predictors_sim.unsqueeze(dim=1)  # neighbours along axis 1, non-neighbours along 2
        #
        # reg_loss_mat = (0.3 - predictors_sim_diff).clamp(min=0)  # 0.3 = margin
        # reg_loss_mat[~adj.unsqueeze(dim=2).expand_as(reg_loss_mat)] = 0
        # reg_loss_mat[adj.unsqueeze(dim=1).expand_as(reg_loss_mat)] = 0
        # reg_loss_mat[null_u, :, :] = 0
        # reg_loss_mat[:, null, :] = 0
        # reg_loss_mat[:, :, null] = 0
        #
        # reg_loss_mat[arange_u, arange[unseen], :] = 0
        # reg_loss_mat[arange_u, :, arange[unseen]] = 0
        # reg_loss_mat[:, arange, arange] = 0
        # # reg_loss_mat = reg_loss_mat[unseen, :, :]
        # reg_loss = reg_loss_mat.sum() / (reg_loss_mat != 0).sum().item()
...
class LogicBranch(PartStateInReprBranch):
    ...
    def _get_losses(self, x: Minibatch, output, **kwargs):
        ...
        losses[f'{loss_prefix}_loss_nec'] = w_nec * w_nec_neg * bce_loss(nec_logits[:, nec_mask], nec_labels[:, nec_mask], pos_weights=1 / w_nec_neg)

        w_suf = cfg.awsls
        w_suf_pos = cfg.awsls_pos
        assert w_suf_pos >= 1
        suf_labels = (symstate_labels @ self.suf_label_mat.t()).clamp(max=1)
        losses[f'{loss_prefix}_loss_suf'] = w_suf * bce_loss(suf_logits[:, suf_mask], suf_labels[:, suf_mask], pos_weights=w_suf_pos)
    ...
    def _forward(self, x: Minibatch):
        repr0 = self.repr_mlps['repr'](self.input_vec_f(x))
        dir_logits = zs_logits = None
        if self.dir_enabled:
...
class AttBranch(PartStateInReprBranch):
    ...
    def _get_losses(self, x: Minibatch, output, **kwargs):
        ...
        losses[f'{self.ld.tag}_att_w_sparsity'] = cfg.att_sp_c * (1 - att_w.max(dim=1)[0].mean())
        return losses
...
class LateFusionAttBranch(LateFusionBranch):
    def __init__(self, *args, **kwargs):
        ...
        self.mha = nn.MultiheadAttention(embed_dim=self.att_emb_dim, num_heads=self.num_att_head, kdim=self.att_emb_dim, vdim=self.repr_dims[0])

        for k, d in self._get_feat_vects(x=None).items():
            self._add_mlp(name=f'att_{k}', input_dim=d, output_dim=self.att_emb_dim)
        self._add_mlp(name=f'att_query', input_dim=self.input_vec_dim, output_dim=self.att_emb_dim)
    ...
    def _forward(self, x: Minibatch):
        ...
        query = self.repr_mlps['att_query'](self.input_vec_f(x)).unsqueeze(dim=0)  # 1 x N x d
