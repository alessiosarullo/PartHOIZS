...
class AbstractBranch(nn.Module):
    def __init__(self, dataset: HicoHakeKPSplit, cache: Cache, repr_dim=cfg.repr_dim, enable_zs=True, **kwargs):
        super().__init__()
...
class PartStateBranch(AbstractBranch):
    def __init__(self, *args, **kwargs):
        ...
        for i, p_states in enumerate(hicohake.states_per_part):
            ...
            if self.final_part_repr_dim is None:
                ...
                assert self.final_part_repr_dim == logit_input_dim
            self._add_linear_layer(name=f'img_part_{part_name}', input_dim=logit_input_dim, output_dim=p_states.size - 1)

    def _get_losses(self, x, output, **kwargs):
        img_data, person_data, obj_data, orig_labels, orig_part_labels, other = x
        part_dir_logits = output
        losses = {'fg_part_loss': bce_loss(part_dir_logits, orig_part_labels[:, self.fg_part_states],
                                           pos_weights=cfg.cspc_p if cfg.cspc_p > 0 else None)}
    ...
    def _predict(self, x, output, **kwargs):
        ...
        if part_dir_logits is not None:
            part_state_scores[:, self.fg_part_states] = torch.sigmoid(part_dir_logits).cpu().numpy()
...
class PretrainedObjectBranch(AbstractBranch):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        if len(self.dataset.seen_objects) < self.dataset.full_dataset.num_objects:
    ...
    def _predict(self, x, output, **kwargs):
        raise NotImplementedError  # FIXME assumes prediction on single image. Fix or delete.
        # obj_data = x[2]
        # obj_boxes, obj_scores, obj_feats = obj_data[:3]
